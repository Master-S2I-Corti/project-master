...
public function save2(Request $request)//////insert la maquete dans la bdd
{
	...
	
	{
		...
		foreach($semestre->ues as $arrue)//////pour chaque ue
		{
			...
			//print_r($ue);
			//echo $sem;
			DB::table('ue')->insert(['id_ue'=>$Nsem.'ue'.$sem,'libelle'=>$ue->nom,'id_semestre'=>$Nsem,'coeff'=>$ue->coeff,'description'=>$ue->description,/*'id_responsable'=>$ue->responsable->id,*/'edts'=>$ue->ects]);
			$nmat=0;
			echo"test";
